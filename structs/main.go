	...
	for option != "5" {
		...
		if option == "4" {
			...
			if !found {
				...
				fmt.Printf("Ucestalost unete vrednosti u CountMinSketch-u: %v", frequency)
			}
		}
		if option == "5" {
			break
...
func meni(wal *wal_implementation.WriteAheadLog, lru1 *lru.LRUCache, memtable *cursor.Cursor, tokenb *token_bucket.TokenBucket) {
	for true {
		...
		if opcija == "1" {
			...
			fmt.Println("Unesite koju sstabele-u zelite da proverite(path) >> ")
			...
			if err != nil {
				fmt.Println("\nIzabrana sstabele-a ne posotji.")
			} else {
